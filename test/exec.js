...
const ORIG_EXEC_PATH = common.execPath;
shell.config.silent = true;

test.afterEach.always(() => {
  process.chdir(CWD);
  common.execPath = ORIG_EXEC_PATH;
});
...
test('exec exits gracefully if we cannot find the execPath', t => {
  common.execPath = null;
  shell.exec('echo foo');
  t.regex(
    shell.error(),
    /Unable to find a path to the node binary\. Please manually set common\.execPath/
  );
});
...
test('check if stdout goes to output', t => {
  const result = shell.exec(`${JSON.stringify(common.execPath)} -e "console.log(1234);"`);
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('check if stderr goes to output', t => {
  const result = shell.exec(`${JSON.stringify(common.execPath)} -e "console.error(1234);"`);
  t.falsy(shell.error());
  t.is(result.code, 0);
  t.is(result.stdout, '');
...
test('check if stdout + stderr go to output', t => {
  const result = shell.exec(`${JSON.stringify(common.execPath)} -e "console.error(1234); console.log(666);"`);
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('check exit code', t => {
  const result = shell.exec(`${JSON.stringify(common.execPath)} -e "process.exit(12);"`);
  t.truthy(shell.error());
  t.is(result.code, 12);
});
...
test('interaction with cd', t => {
  ...
  const result = shell.exec(`${JSON.stringify(common.execPath)} node_script.js`);
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('check quotes escaping', t => {
  const result = shell.exec(util.format(JSON.stringify(common.execPath) + ' -e "console.log(%s);"', "\\\"\\'+\\'_\\'+\\'\\\""));
  t.falsy(shell.error());
  t.is(result.code, 0);
...
test('set timeout option', t => {
  const result = shell.exec(`${JSON.stringify(common.execPath)} resources/exec/slow.js 100`); // default timeout is ok
  t.falsy(shell.error());
  t.is(result.code, 0);
  if (process.version >= 'v0.11') {
    // this option doesn't work on v0.10
    shell.exec(`${JSON.stringify(common.execPath)} resources/exec/slow.js 100`, { timeout: 10 }); // times out
...
test.cb('no callback', t => {
  const c = shell.exec(`${JSON.stringify(common.execPath)} -e "console.log(1234)"`, { async: true });
  t.falsy(shell.error());
...
test.cb('callback as 2nd argument', t => {
  shell.exec(`${JSON.stringify(common.execPath)} -e "console.log(5678);"`, (code, stdout, stderr) => {
    t.is(code, 0);
    t.is(stdout, '5678\n');
    t.is(stderr, '');
    t.end();
...
test.cb('callback as end argument', t => {
  shell.exec(`${JSON.stringify(common.execPath)} -e "console.log(5566);"`, { async: true }, (code, stdout, stderr) => {
    t.is(code, 0);
    t.is(stdout, '5566\n');
    t.is(stderr, '');
    t.end();
...
test.cb('callback as 3rd argument (silent:true)', t => {
  shell.exec(`${JSON.stringify(common.execPath)} -e "console.log(5678);"`, { silent: true }, (code, stdout, stderr) => {
    t.is(code, 0);
    t.is(stdout, '5678\n');
    t.is(stderr, '');
    t.end();
