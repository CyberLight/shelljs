...
test('default behavior', t => {
  const result = shell.exec(JSON.stringify(common.execPath) + ' -e "require(\'../global\'); ls(\'file_doesnt_exist\'); echo(1234);"');
  t.is(result.code, 0);
...
test('set -e', t => {
  const result = shell.exec(JSON.stringify(common.execPath) + ' -e "require(\'../global\'); set(\'-e\'); ls(\'file_doesnt_exist\'); echo(1234);"');
...
test('set -v', t => {
  const result = shell.exec(JSON.stringify(common.execPath) + ' -e "require(\'../global\'); set(\'-v\'); ls(\'file_doesnt_exist\'); echo(1234);"');
  t.is(result.code, 0);
  t.is(result.stdout, '1234\n');
  t.is(
    result.stderr,
...
test('set -ev', t => {
  const result = shell.exec(JSON.stringify(common.execPath) + ' -e "require(\'../global\'); set(\'-ev\'); ls(\'file_doesnt_exist\'); echo(1234);"');
  t.is(result.code, uncaughtErrorExitCode);
  t.is(result.stdout, '');
  t.truthy(result.stderr.indexOf('Error: ls: no such file or directory: file_doesnt_exist') >= 0);
...
test('set -e, set +e', t => {
  const result = shell.exec(JSON.stringify(common.execPath) + ' -e "require(\'../global\'); set(\'-e\'); set(\'+e\'); ls(\'file_doesnt_exist\'); echo(1234);"');
  t.is(result.code, 0);
  t.is(result.stdout, '1234\n');
  t.is(result.stderr, 'ls: no such file or directory: file_doesnt_exist\n');
});
